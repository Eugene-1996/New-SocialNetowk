    ...
    dispatch(action: any) {
        ...
        if (action.type === 'ADD-POST') {
            let newPost = {
                ...
                name: 'Kiril',
                img: 'https://images.pexels.com/photos/614810/pexels-photo-614810.jpeg?cs=srgb&dl=pexels-simon-robben-614810.jpg&fm=jpg',
                description: this.getState().profileData.newPost
            }
            this.getState().profileData.posts.push(newPost)
            this.getState().profileData.newPost = ''
            this._callSubscriber(this._state)
        }
        else if (action.type === 'SET-NEW-POST') {
            this.getState().profileData.newPost = action.newPost
            this._callSubscriber(this._state)
        }
        else if (action.type === 'ADD-MESSAGE') {
            let newMessage = {
                id: String(Math.floor(Math.random() * 12234)),
                message: this.getState().dialogsData.newMessage
            }
            this.getState().dialogsData.messages.push(newMessage)
            this.getState().dialogsData.newMessage = ''
            this._callSubscriber(this._state)
        }
        else if (action.type === 'SET-NEW-MESSAGE') {
            this._state.dialogsData.newMessage = action.newMessage
            this._callSubscriber(this._state)
        }
    }
}
